...
import { getPrices, getProducts, createPrice, updatePrice, deletePrice, getPriceTypes } from '../../api';
import {
  ChevronDown,
  ChevronRight,
  List,
...
const PricesPage = () => {
  ...
  const [totalPricesCount, setTotalPricesCount] = useState(0);
  ...
  const debounceGet = useDebounce(async () => {
        ...
        if (searchQuery || Object.keys(filters).length > 0) {
          pricesRes = await getPrices({
            searchQuery,
            categoryId: filters.category_id,
            priceFloor: filters.price_floor,
            priceCeiling: filters.price_ceiling,
            priceTypeId: filters.price_type_id,
            offset: (currentPage-1)*pricesPerPage,
            limit: pricesPerPage,
            sortBy,
            sortOrder
          });
        ...
        if (pricesRes.data.total < currentPage*20) {
          setCurrentPage(1);
        }
        const typesRes = await getPriceTypes();
        setPrices(pricesRes.data.prices || []);
        setTotalPricesCount(pricesRes.data.total || 0);
        setPriceTypes(typesRes.data || []);
      } catch (err) {
        console.error('Error fetching prices:', err);
        setError('Ошибка при загрузке цен');
      } finally {
        setLoading(false);
      }
  }, 300);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deletePrice(selectedPrice.product_price_id);
      ...
      setTotalPricesCount(res.data.total || 0);
    } catch (err) {
  ...
  const handleSavePrice = async () => {
    ...
    try {
      if (selectedPrice.product_price_id) {
        await updatePrice(selectedPrice.product_price_id, {
          price: selectedPrice.price,
          price_type_id: selectedPrice.price_type_id,
          data_s: selectedPrice.product_dates.data_s,
          data_e: selectedPrice.product_dates.data_e
        });
      } else {
        await createPrice({
      ...
      setError('Ошибка при сохранении цены');
    }
  };
  ...
  const countPricesInCategory = (category: any): number => {
    let count = prices.filter(p => p.product.category_id === category.category_id).length;

    if (category.children) {
      count += category.children.reduce(
        (sum: number, child: any) => sum + countPricesInCategory(child),
        0
      );
    }
  ...
  const renderCategoryTree = (categories: any[], level = 0) => {
    return categories.map(category => {
      ...
      const directPrices = prices.filter(p => p.product.category_id === category.category_id);
      const totalPricesInCategory = countPricesInCategory(category);
            ...
            <td colSpan={7} className="px-6 py-3">
              <div className="flex items-center" style={{ paddingLeft: `${level * 20}px` }}>
                ...
                <span className="ml-2 text-sm text-gray-500">
                  ({totalPricesInCategory})
                </span>
              </div>
            </td>
          </tr>

          {isExpanded && directPrices.map(price => (
            <PriceRow
              key={`price-${price.product_price_id}-${category.category_id}`}
              price={price}
    ...
    <div className="p-6">
                ...
                <th className="w-32 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <div className="flex items-center">
                    ...
                    <button onClick={() => handleSort('price')} className="ml-2">
                      {sortBy === 'price' ? (sortOrder === 'asc' ? <ArrowUp size={16} /> : <ArrowDown size={16} />) : <ArrowUp size={16} className="text-gray-400" />}
                    </button>
                  </div>
                </th>
                <th className="w-48 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <div className="flex items-center">
                    Тип цены
                    <button onClick={() => handleSort('price_type')} className="ml-2">
                      {sortBy === 'price_type' ? (sortOrder === 'asc' ? <ArrowUp size={16} /> : <ArrowDown size={16} />) : <ArrowUp size={16} className="text-gray-400" />}
                    </button>
                  </div>
                </th>
                <th className="w-32 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
      ...
      {!showHierarchicalView && (
        <Pagination
          ...
          totalPages={Math.ceil(totalPricesCount / pricesPerPage)}
          onPageChange={setCurrentPage}
          hasMore={currentPage < Math.ceil(totalPricesCount / pricesPerPage)}
        />
      )}
      ...
      <ConfirmationModal
        ...
        title="Удаление цены"
        message={`Вы действительно хотите удалить цену для товара "${selectedPrice?.product.name}"?`}
        confirmText="Удалить"
        cancelText="Отмена"
        isDanger
      />

      {/* Модальное окно редактирования/добавления цены */}
      {editModalOpen && selectedPrice && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="p-6">
              <h2 className="text-xl font-bold mb-4">
                {selectedPrice.product_price_id ? 'Редактировать цену' : 'Добавить новую цену'}
              </h2>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div>
                   {selectedPrice.product_price_id ? (
                      <input
                        type="text"
                        value={selectedPrice.product.name || ''}
                        readOnly
                      ...
                      >
                        {selectedPrice?.product?.name || 'Выберите товар'}
                      </button>
                      )}
                ...
                <div>
                  ...
                  <select
                    value={selectedPrice?.price_type_id || ''}
                    onChange={(e) => {
                      if (!selectedPrice) return;
                      setSelectedPrice({
                        ...selectedPrice,
                        price_type_id: parseInt(e.target.value)
                      });
                    }}
                    className="w-full border rounded-lg p-2"
                    required
                ...
                <div>
                  ...
                  <DatePicker
                    selected={selectedPrice.product_dates.data_s ? new Date(selectedPrice.product_dates.data_s) : new Date()}
                    onChange={(date) => setSelectedPrice({
                      ...selectedPrice,
                      product_dates: {
                        ...selectedPrice.product_dates,
                        data_s: date?.toISOString() || selectedPrice.product_dates.data_s
                      }
                    })}
                    className="w-full border rounded-lg p-2"
                ...
                <div>
                  ...
                  <DatePicker
                    selected={selectedPrice.product_dates.data_e ? new Date(selectedPrice.product_dates.data_e) : new Date()}
                    onChange={(date) => setSelectedPrice({
                      ...selectedPrice,
                      product_dates: {
                        ...selectedPrice.product_dates,
                        data_e: date?.toISOString() || selectedPrice.product_dates.data_e
                      }
                    })}
                    className="w-full border rounded-lg p-2"
                    dateFormat="dd.MM.yyyy"
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                >
                  {selectedPrice.product_price_id ? 'Сохранить' : 'Добавить'}
                </button>
              </div>
            </div>
          </div>
...
const PriceRow = ({ price, onDeleteClick, onEditClick }: {
  price: Price;
  onDeleteClick: (price: Price) => void;
  onEditClick: (price: Price) => void;
}) => {
  const {user} = useAuth();
  ...
  return (
    <tr className="hover:bg-gray-50">
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        <Link
        to={`/products/${price.product_id}`}
        className="text-primary-600 hover:text-primary-900"
      >
        {price.product.article}
      </Link>
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        {(user?.role.admin || user?.role.moderator) ? (
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        {price.product_dates.data_s}
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
        {price.product_dates.data_e}
      </td>
      {(user?.role.admin || user?.role.moderator) && (
